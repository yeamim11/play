  ...
  return (
    <div className='bg-black text-white p-4 md:p-6 relative'>
      <nav className='flex justify-between items-center mx-auto'>
        ...
        <div className='flex justify-between items-center gap-4 md:gap-10 font-semibold'>
          <ul className="hidden md:flex justify-between items-center gap-10 text-white">
            <li className="text-lg font-semibold hover:text-[#FE7C45] hover:underline hover:underline-offset-4">
              <Link to="/inicio">Inicio</Link>
            </li>
            <li className="text-lg font-semibold hover:text-[#FE7C45] hover:underline hover:underline-offset-4">
              <Link to="/contacto">Contacto</Link>
            ...
            <button
              onClick={openLogin}
              className="px-5 py-1 bg-[#fe7c45] text-black text-lg font-semibold hover:scale-105 cursor-pointer"
            >
              Login
            </button>
          </ul>
        ...
        <div className={`fixed top-0 right-0 w-64 h-screen bg-[#FE7C45]/90 text-white flex flex-col items-start gap-6 p-6 transform ${menuOpen ? "translate-x-0" : "translate-x-full"} transition-transform duration-500 md:hidden`}>
          ...
          <ul className="mt-20 space-y-4">
            <li><Link className="text-lg font-semibold" to="/inicio" onClick={() => setMenuOpen(false)}>Inicio</Link></li>
            <li><Link className="text-lg font-semibold" to="/contacto" onClick={() => setMenuOpen(false)}>Contacto</Link></li>
            <li><Link className="text-lg font-semibold" to="/tienda" onClick={() => setMenuOpen(false)}>Tienda</Link></li>
          </ul>
          <button
            onClick={() => {
              setMenuOpen(false);
            ...
            className="px-5 py-1 bg-[#090909] text-white text-lg font-semibold hover:scale-105 cursor-pointer"
          >
            Login
          </button>
        </div>
      ...
      {/* Modals */}
      {showLogin && <Login onClose={closeModals} switchToRegister={openRegister} />}
